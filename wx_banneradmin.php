...
<script>
$.get('api.php?action=get_wx_banner_list',function(d){
	var d = d.data;
	var html='';
...
$('#add_new_banner_button').click(function(){
	...
	$.post('api.php?action=add_wx_banner',{
		pos:pos,
		desc:desc,
		title:title,
		link:link,
...
$(document).delegate('.del_banner','click',function(){
	...
	$.post('api.php?action=del_wx_banner',{
		index:index
	},function(d){
		if(d == 'success'){
			alert('添加成功');
...
var Upload = function(opt){
	  ...
	  $('#review').click(function(){
		location.href="http://touch.n.mescake.com/main";
	  });
</script>
</html>